...
const ASSIGNMENT_ID_A = '11111111-1111-4111-8111-111111111111';
const ASSIGNMENT_ID_B = '22222222-2222-4222-8222-222222222222';
...
test('readInput', async (t) => {
	...
	await t.test(
		'preserves hyphens: INPUT_TARGETS-FILE for "targets-file"',
		() => {
...
test('requireEnv', async (t) => {
	...
	await t.test('throws with var name in message when unset', () => {
		...
		assert.throws(
			() => requireEnv('TEST_MISSING_ENV_XYZ'),
			(err) => {
...
test('validateEntry', async (t) => {
	await t.test('accepts valid entry', () => {
		assert.doesNotThrow(() =>
			validateEntry({ target: { k: 'v' }, wait: true }, 0, 'src'),
		);
		assert.doesNotThrow(() =>
			validateEntry({ target: {}, wait: false }, 0, 'src'),
		);
	});
	...
	await t.test('rejects bad target', () => {
		assert.throws(
			() => validateEntry({ target: null, wait: true }, 1, 'src'),
			/target/,
		);
		assert.throws(
			() => validateEntry({ target: [], wait: true }, 1, 'src'),
			/target/,
		);
		assert.throws(
			() => validateEntry({ target: 'x', wait: true }, 1, 'src'),
			/target/,
		);
	});
	await t.test('rejects non-boolean wait when set', () => {
		assert.throws(
			() => validateEntry({ target: {}, wait: 'true' }, 2, 'src'),
			/wait/,
		);
		assert.throws(
			() => validateEntry({ target: {}, wait: 1 }, 2, 'src'),
			/wait/,
		);
		assert.throws(
			() => validateEntry({ target: {}, wait: null }, 2, 'src'),
			/wait/,
		);
	});
});
...
test('resolveTargets', async (t) => {
	...
	await t.test('throws when both set', () => {
		assert.throws(
			() => resolveTargets(TARGETS_JSON, '/some/path'),
			/mutually exclusive/,
		);
	});
	...
	await t.test('throws when file does not exist', () => {
		assert.throws(
			() => resolveTargets('', '/nonexistent/path/xyz.json'),
...
test('formatTarget', () => {
	...
	assert.equal(
		formatTarget({ kind: 'management', tenant: 'nav' }),
		'{kind: management, tenant: nav}',
	);
...
test('fetchOidcToken', async (t) => {
	await t.test('returns token on success and checks auth header', async () => {
		...
		const result = await fetchOidcToken(
			`http://127.0.0.1:${port}`,
			'test-request-token',
		);
	...
	await t.test('throws when value is null', async () => {
		...
		const port = server.address().port;
		await assert.rejects(() =>
			fetchOidcToken(`http://127.0.0.1:${port}`, 'tok'),
		);
	});
});
...
test('postAssignment', async (t) => {
	await t.test(
		'sends correct headers and body, returns id from response',
		async () => {
			...
			const server = await startServer((req, res) => {
				...
				req.on('end', () => {
					...
					res.end(JSON.stringify({ id: ASSIGNMENT_ID_A }));
				});
			});
			t.after(() => new Promise((r) => server.close(r)));
			const port = server.address().port;
			...
			const id = await postAssignment(
				`http://127.0.0.1:${port}`,
				TOKEN,
				payload,
			);
			assert.equal(id, ASSIGNMENT_ID_A);
	...
	await t.test('throws on 500 with status and body but not token', async () => {
		...
		const port = server.address().port;
		await assert.rejects(
			() => postAssignment(`http://127.0.0.1:${port}`, TOKEN, {}),
			(err) => {
...
test('fetchAssignmentStatus', async (t) => {
...
test('pollAssignmentStatus', async (t) => {
	await t.test('resolves on DEPLOYED', async () => {
		...
		t.after(() => new Promise((r) => server.close(r)));
		const port = server.address().port;
		const result = await pollAssignmentStatus(
			`http://127.0.0.1:${port}`,
			async () => TOKEN,
			ASSIGNMENT_ID_A,
	...
	await t.test('keeps polling through PENDING then DEPLOYED', async () => {
		...
		const server = await startServer((req, res) => {
			res.writeHead(200, { 'Content-Type': 'application/json' });
			res.end(
				JSON.stringify({ state: sequence[Math.min(i++, sequence.length - 1)] }),
			);
		});
		t.after(() => new Promise((r) => server.close(r)));
		const port = server.address().port;
...
test('describeFetchError', async (t) => {
	...
	await t.test('includes cause message and code', () => {
		const cause = Object.assign(new Error('getaddrinfo ENOTFOUND fasit'), {
			code: 'ENOTFOUND',
		});
...
test('fetchWithRetry', async (t) => {
	await t.test(
		'returns response on first success without retrying',
		async () => {
			let calls = 0;
	...
	await t.test(
		'throws wrapped error after exhausting attempts and includes cause',
		async () => {
			let calls = 0;
			await assert.rejects(
				() =>
					fetchWithRetry(
						...
						async () => {
							calls++;
							const err = new TypeError('fetch failed');
							err.cause = Object.assign(new Error('connect ETIMEDOUT'), {
								code: 'ETIMEDOUT',
							});
							throw err;
						},
	...
	await t.test(
		'does not retry on HTTP error responses (caller decides)',
		async () => {
			let calls = 0;
			const res = await fetchWithRetry(
				'test',
...
test('fetchAssignmentStatus retries transient fetch errors', async (t) => {
	await t.test('recovers when fetch throws then server responds', async () => {
		...
		assert.equal(
			httpCalls,
			1,
			'server should have been hit once after retries',
		);
	});
});
...
function mockFasitHandler({
	...
	return (req, res) => {
		...
		if (req.method === 'POST') {
			...
			req.on('end', () => {
				const id = postCounter === 0 ? ASSIGNMENT_ID_A : ASSIGNMENT_ID_B;
				postCounter++;
				if (capturedPosts)
					capturedPosts.push({ id, payload: JSON.parse(body) });
				res.writeHead(201, { 'Content-Type': 'application/json' });
		...
		if (req.method === 'GET') {
			const idMatch = req.url.match(/\/github\/assignment\/([^/?]+)/);
...
test('main() happy path posts once per target (no wait)', async (t) => {
	...
	process.env['INPUT_ENDPOINT'] =
		`http://127.0.0.1:${fasitServer.address().port}`;
	...
	process.env['ACTIONS_ID_TOKEN_REQUEST_URL'] =
		`http://127.0.0.1:${oidcServer.address().port}`;
	...
	assert.ok(
		!process.exitCode,
		`exitCode should be 0/undefined, got ${process.exitCode}`,
	);

	assert.equal(capturedPosts.length, 2, 'should POST once per target');
	const expectedPayloadA = buildPayload({
		chart: CHART,
		version: VERSION,
		target: TARGET_A,
		owner: OWNER,
	...
	const expectedPayloadB = buildPayload({
		chart: CHART,
		version: VERSION,
		target: TARGET_B,
		owner: OWNER,
	...
	assert.equal(
		capturedPosts[0].payload.ci,
		undefined,
		'payload must not contain ci',
	);
	...
	assert.ok(summary.includes('### Assignment created!'));
	assert.ok(summary.includes('kind: management'));
	assert.ok(summary.includes('tenant: ci'));
	assert.ok(summary.includes('tenant: nav'));
	assert.ok(summary.includes('fasit.nais.io/assignments'));
	assert.ok(
		summary.includes(`https://fasit.nais.io/assignments/${ASSIGNMENT_ID_A}`),
		'summary should link to fasit assignment page per assignment',
	);
});
...
test('main() polls when wait=true and resolves on DEPLOYED', async (t) => {
	...
	fasitServer = await startServer(
		mockFasitHandler({ capturedPosts, capturedPaths, postState: 'DEPLOYED' }),
	);
	...
	assert.ok(
		getCalls.length >= 1,
		'should have polled at least once for the wait=true target',
	);
	assert.ok(
		getCalls.some((p) => p.includes(ASSIGNMENT_ID_A)),
		'should have polled the wait=true assignment id',
	);
	assert.ok(
		!getCalls.some((p) => p.includes(ASSIGNMENT_ID_B)),
		'should NOT poll the wait=false target',
	);
});
...
test('main() exits non-zero when polling sees FAILED', async (t) => {
	...
	fasitServer = await startServer(
		mockFasitHandler({ capturedPosts, postState: 'FAILED' }),
	);
	...
	process.env['INPUT_CHART'] = CHART;
	process.env['INPUT_VERSION'] = VERSION;
	process.env['INPUT_TARGETS'] = JSON.stringify([
		{ target: TARGET_A, wait: true },
		{ target: TARGET_B, wait: true },
	]);
	delete process.env['INPUT_TARGETS-FILE'];
	process.env['GITHUB_REPOSITORY'] = REPO_FULL;
	process.env['GITHUB_REPOSITORY_OWNER'] = OWNER;
	...
	assert.equal(
		...
		1,
		'should abort after first failed poll, not POST second target',
	);
	...
	assert.ok(
		summary.includes(`https://fasit.nais.io/assignments/${ASSIGNMENT_ID_A}`),
		'summary should link to fasit assignment page on failure',
	);
});
...
test('main() reads targets-file', async (t) => {
	...
	fs.writeFileSync(
		tmpFile,
		JSON.stringify([{ target: TARGET_A, wait: false }]),
	);
...
test('main() aborts on first POST failure', async (t) => {
	...
	assert.equal(
		postCount,
		1,
		'should abort after first failure, not POST again',
	);
});
...
test('smoke: subprocess failure paths', async (t) => {
	...
	const cases = [
		{
			...
			env: { ...baseEnv, ACTIONS_ID_TOKEN_REQUEST_TOKEN: '' },
			keyword: 'ACTIONS_ID_TOKEN_REQUEST_TOKEN',
			file: 'task-2-failure-1.log',
		},
		{
			name: 'empty chart',
			env: { ...baseEnv, INPUT_CHART: '' },
			keyword: 'chart',
			file: 'task-2-failure-2.log',
		},
		{
			name: 'empty version',
			env: { ...baseEnv, INPUT_VERSION: '' },
			keyword: 'version',
			file: 'task-2-failure-3.log',
		},
		{
			name: 'invalid targets JSON',
			env: { ...baseEnv, INPUT_TARGETS: 'not-json' },
			keyword: 'targets',
			file: 'task-2-failure-4.log',
		},
		{
			name: 'targets is not an array',
			env: { ...baseEnv, INPUT_TARGETS: '{}' },
			keyword: 'array',
			file: 'task-2-failure-5.log',
		},
		{
			name: 'empty targets array',
			env: { ...baseEnv, INPUT_TARGETS: '[]' },
			keyword: 'at least one',
			file: 'task-2-failure-6.log',
		},
		{
			name: 'neither targets nor targets-file',
			env: { ...baseEnv, INPUT_TARGETS: '' },
			keyword: 'targets',
			file: 'task-2-failure-7.log',
		},
	];
	...
	for (const c of cases) {
		await t.test(c.name, async () => {
			...
			assert.ok(
				stderr.toLowerCase().includes(c.keyword.toLowerCase()),
				`Expected "${c.keyword}" in stderr for: ${c.name}. Got: ${stderr}`,
			);
		});
	}
	...
	await t.test('fasit returns 500', async () => {
		const failFasit = await startServer((req, res) => {
			res.writeHead(500);
		...
		const env = {
			...baseEnv,
			INPUT_ENDPOINT: `http://127.0.0.1:${failFasit.address().port}`,
		};
		const { code, stderr } = await spawnScript(env);
		fs.writeFileSync(
			path.join(evidenceDir, 'task-2-failure-fasit500.log'),
			stderr,
		);
		assert.notEqual(code, 0);
	...
	await t.test('oidc returns 401', async () => {
		const failOidc = await startServer((req, res) => {
			res.writeHead(401);
			res.end('unauthorized');
		});
		...
		const env = {
			...baseEnv,
			ACTIONS_ID_TOKEN_REQUEST_URL: `http://127.0.0.1:${failOidc.address().port}`,
		};
		const { code, stderr } = await spawnScript(env);
		fs.writeFileSync(
			path.join(evidenceDir, 'task-2-failure-oidc401.log'),
			stderr,
		);
		assert.notEqual(code, 0);
